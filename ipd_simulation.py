...
# %% Clase Evolucion: Gestiona el proceso evolutivo
class Evolucion:
    ...
    def inicializar_poblacion(self, n_jugadores, tipos_jugador): # Crea la poblacion inicial
        ...
        for i in range(n_jugadores): # Repite por cada jugador a crear
            tipo = tipos_jugador[i % len(tipos_jugador)] # Selecciona tipo de forma ciclica
            memoria = tipo[0] # Extrae la profundidad de memoria
            arquitectura = tipo[1] if len(tipo) > 1 else [] # Extrae la arquitectura o usa [] por defecto
            jugador = Jugador(memoria, arquitectura) # Crea el jugador
            self.poblacion.append(jugador) # Agrega el jugador a la poblacion
    ...
    def seleccion(self, puntuaciones, top_n=50): # Selecciona los mejores jugadores
        indices = np.argsort(puntuaciones)[::-1] # Ordena indices por puntuacion descendente
        self.poblacion = [self.poblacion[i] for i in indices[:top_n]] # Mantiene solo los top N

    def reproducir_poblacion(self, desviacion_std=0.3): # Crea nueva generacion mediante clonacion con ruido
        nueva_poblacion = [] # Lista para la nueva poblacion
        for i in range(len(self.poblacion)): # Repite hasta completar la poblacion
            padre = self.poblacion[i % len(self.poblacion)] # Selecciona un padre
            hijo = padre.reproducir(desviacion_std) # Crea un hijo clonando al padre con ruido gaussiano
            nueva_poblacion.append(hijo) # Agrega el hijo a la nueva poblacion
    ...
    def ejecutar_generacion(self, grabador): # Ejecuta un ciclo completo de generacion
        ...
        self.seleccion(puntuaciones) # Selecciona los mejores
        self.reproducir_poblacion() # Crea nueva generacion
...
if __name__ == "__main__": # Si se ejecuta como programa principal
    print("=" * 80)
    print("SIMULACION DEL DILEMA DEL PRISIONERO ITERADO")
    print("=" * 80)
    ...
    tipos = [
        ...
        #(4, [6]),      # Memoria 4,
        #(4, [20]),     # Memoria 5,
        (4, [20, 4]),  # Memoria 4, dos capas ocultas con 8 y 4 neuronas (profunda)
    ]
